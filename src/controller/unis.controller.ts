    ...
    const rows = await cache.remember(
      ...
      async () => {
        return await db.query(`SELECT s.structure_code, CONCAT(s.structure_code, '-', ss.facing_no, ss.transformation, LPAD(ss.segment, 2, '0')) as site_code, A.net_contract_amount, A.payment_term_id FROM (
          ...
          JOIN hd_structure_segment ss ON s.structure_id = ss.structure_id GROUP BY ss.segment_id;
              `);
      },
    );
...
WHERE s.product_division_id = 1 AND ss.transformed = 0 AND st.status_id IN (1,2) ORDER BY s.structure_id DESC ) A WHERE DATE(date_created) >= CURDATE() - INTERVAL 120 DAY ORDER BY date_created DESC`,
      [date],
    );
