...
WHERE s.product_division_id = 1 AND ss.transformed = 0 AND st.status_id IN (1,2) ORDER BY s.structure_id DESC ) A WHERE DATE(date_created) >= CURDATE() - INTERVAL 6 MONTH ORDER BY date_created DESC`,
      [date],
    );
